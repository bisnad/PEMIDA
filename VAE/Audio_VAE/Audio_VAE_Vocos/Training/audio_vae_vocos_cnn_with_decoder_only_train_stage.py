...
encoder_weights_file = "results_Diane_audio_vae_vocos_cnn_ld128_kld1.0/weights/encoder_weights_epoch_400"
decoder_weights_file = "results_Diane_audio_vae_vocos_cnn_ld128_kld1.0/weights/decoder_weights_epoch_400"

encoder_weights_file = "results_Diane_audio_vae_vocos_cnn_ld32_kld1.0_decoder_training/weights/encoder_weights_epoch_400"
decoder_weights_file = "results_Diane_audio_vae_vocos_cnn_ld32_kld1.0_decoder_training/weights/decoder_only_weights_epoch_200"
...
encoder_weights_file = "results_Eleni_audio_vae_vocos_cnn_ld128_kld1.0/weights/encoder_weights_epoch_400"
decoder_weights_file = "results_Eleni_audio_vae_vocos_cnn_ld128_kld1.0/weights/decoder_weights_epoch_400"
...
encoder_weights_file = "results_Tim_audio_vae_vocos_cnn_ld128_kld1.0/weights/encoder_weights_epoch_400"
decoder_weights_file = "results_Tim_audio_vae_vocos_cnn_ld128_kld1.0/weights/decoder_weights_epoch_400"

"""

encoder_weights_file = "results_Tim_audio_vae_vocos_cnn_ld32_kld1.0_decoder_training/weights/encoder_weights_epoch_400"
decoder_weights_file = "results_Tim_audio_vae_vocos_cnn_ld32_kld1.0_decoder_training/weights/decoder_only_weights_epoch_200"
...
ae_dec_only_learning_rate = ae_learning_rate
...
"""
Training First Pass (full autoencoder)
"""
...
if save_weights == True:
    ...
    save_loss_as_csv(loss_history, "results/histories/history_first_pass_{}.csv".format(epochs))
    save_loss_as_image(loss_history, "results/histories/history_first_pass_{}.png".format(epochs))
...
"""
Training Second Pass: encoder only
"""
...
# new optimizer that only sees decoder params
dec_optimizer = torch.optim.Adam(decoder.parameters(), lr=ae_dec_only_learning_rate)
dec_scheduler = torch.optim.lr_scheduler.StepLR(dec_optimizer, step_size=50, gamma=0.316)

def decoder_only_train_step(y_wave):
    batch_size = y_wave.shape[0]
    y_mels = vocos.feature_extractor(y_wave)

    # regroup mels
    y_mels_regrouped = y_mels.reshape(
        batch_size, 1, audio_mel_filter_count, audio_vae_mels_per_vocos_mels, audio_mel_count_vae
    )
    y_mels_regrouped = y_mels_regrouped.permute(0, 3, 1, 2, 4)
    y_mels_regrouped = y_mels_regrouped.reshape(-1, 1, audio_mel_filter_count, audio_mel_count_vae)

    # encoder forward (frozen, no grad)
    with torch.no_grad():
        audio_encoder_out_mu, audio_encoder_out_std = encoder(y_mels_regrouped)
        mu = audio_encoder_out_mu
        std = torch.nn.functional.softplus(audio_encoder_out_std) + 1e-6
    ...
    # mel reconstruction loss (no KL term here, encoder is frozen)
    _ae_mel_rec_loss = ae_mel_loss(y_mels_regrouped, yhat_mels_regrouped)

    # regroup predicted mels back to full vocos sequence
    yhat_mels = yhat_mels_regrouped.reshape(
        batch_size, audio_vae_mels_per_vocos_mels, 1, audio_mel_filter_count, audio_mel_count_vae
    )
    yhat_mels = yhat_mels.permute(0, 2, 3, 1, 4)
    yhat_mels = yhat_mels.reshape(batch_size, audio_mel_filter_count, audio_mel_count_vocos)
    ...
    # perceptual loss in waveform domain
    _ae_perc_rec_loss = ae_perc_loss(y_wave, yhat_wave.unsqueeze(1))
    ...
    # backprop only through decoder
    dec_optimizer.zero_grad()
    _total_loss.backward()
    dec_optimizer.step()

    # return scalars for logging (no KL now)
    return _total_loss, _ae_mel_rec_loss, _ae_perc_rec_loss

def train_decoder_only(dataloader, epochs_decoder):
    loss_history_dec = {}
    loss_history_dec["dec train"] = []
    loss_history_dec["dec mel"] = []
    loss_history_dec["dec perc"] = []

    for epoch in range(epochs_decoder):
        start = time.time()
        dec_train_loss_per_epoch = []
        dec_mel_loss_per_epoch = []
        dec_perc_loss_per_epoch = []

        for train_batch in dataloader:
            train_batch = train_batch.unsqueeze(1).to(device)
            _loss, _mel_loss, _perc_loss = decoder_only_train_step(train_batch)

            _loss = _loss.detach().cpu().numpy()
            _mel_loss = _mel_loss.detach().cpu().numpy()
            _perc_loss = _perc_loss.detach().cpu().numpy()

            dec_train_loss_per_epoch.append(_loss)
            dec_mel_loss_per_epoch.append(_mel_loss)
            dec_perc_loss_per_epoch.append(_perc_loss)

        dec_train_loss_per_epoch = np.mean(np.array(dec_train_loss_per_epoch))
        dec_mel_loss_per_epoch = np.mean(np.array(dec_mel_loss_per_epoch))
        dec_perc_loss_per_epoch = np.mean(np.array(dec_perc_loss_per_epoch))

        if epoch % model_save_interval == 0 and save_weights:
            torch.save(decoder.state_dict(), "results/weights/decoder_only_weights_epoch_{}".format(epoch))

        loss_history_dec["dec train"].append(dec_train_loss_per_epoch)
        loss_history_dec["dec mel"].append(dec_mel_loss_per_epoch)
        loss_history_dec["dec perc"].append(dec_perc_loss_per_epoch)

        print(
            "decoder-only epoch {} : train {:01.4f} mel {:01.4f} perc {:01.4f} time {:01.2f}".format(
                epoch + 1,
                dec_train_loss_per_epoch,
                dec_mel_loss_per_epoch,
                dec_perc_loss_per_epoch,
    ...
    return loss_history_dec

# run second pass, e.g. 100 epochs
decoder_only_epochs = 200

if save_weights == True:
    loss_history_dec = train_decoder_only(dataloader, decoder_only_epochs)

    save_loss_as_csv(loss_history, "results/histories/history_second_pass_{}.csv".format(decoder_only_epochs))
    save_loss_as_image(loss_history, "results/histories/history_second_pass_{}.png".format(decoder_only_epochs))

    # save model weights
    torch.save(decoder.state_dict(), "results/weights/decoder_only_weights_epoch_{}".format(decoder_only_epochs))
...
# Eleni Version
test_waveform, _ = torchaudio.load("E:/Data/audio/Eleni/48khz/4_5870821179501060412.wav")
...
test_start_times_sec = [ 5, 50, 100, 140, 160, 214, 270, 345 ]

# Eleni Version
test_waveform, _ = torchaudio.load("E:/Data/audio/Eleni/48khz/4_5870821179501060412.wav")

test_start_times_sec = [ 20, 130, 240, 360, 480, 540, 660, 780 ]

# Tim Versin
test_waveform, _ = torchaudio.load("E:/Data/audio/Tim/48khz/SajuHariPlacePrizeEntry2010.wav")

test_start_times_sec = [5, 25, 54, 90, 106, 226, 290, 320]
"""
...
for i in range(len(test_start_times_sec) - 1):

    test1_start_time_sec = test_start_times_sec[i]
    test2_start_time_sec = test_start_times_sec[i + 1]
