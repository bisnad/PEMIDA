"""
Same as audio_vae_vocos_cnn.py but with different method for creating dataset and proper train and test sets
"""
...
save_weights = False
load_weights = True
encoder_weights_file = "results_Eleni_audio_vae_vocos_cnn_v2_ld32_kld1.0_deconly_offset6400/weights/decoder_only_weights_epoch_200"
decoder_weights_file = "results_Eleni_audio_vae_vocos_cnn_v2_ld32_kld1.0_deconly_offset6400/weights/decoder_weights_epoch_400"

"""
Dataset Settings
"""

audio_waveform_increment = 6400 # 6400
batch_size = 128 # 32
test_percentage = 0.1
...
for sI in range(len(audio_all_data)):
    ...
    with torch.no_grad():
        ...
        for asI in range(0, audio_data.shape[0] - audio_window_length_vocos, audio_waveform_increment):
...
def train(train_loader, test_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for test_batch in test_loader:
            ...
            _ae_loss, _, _, _ = ae_test_step(train_batch)
...
"""
Training Second Pass: decoder only
"""
...
@torch.no_grad()
def decoder_only_test_step(y_wave):
    batch_size = y_wave.shape[0]
    y_mels = vocos.feature_extractor(y_wave)
...
def train_decoder_only(train_loader, test_loader, epochs_decoder):
    loss_history_dec = {}
    loss_history_dec["dec train"] = []
    loss_history_dec["dec test"] = []
    ...
    for epoch in range(epochs_decoder):
        ...
        for test_batch in test_loader:
            test_batch = test_batch.unsqueeze(1).to(device)
            _loss, _, _ = decoder_only_test_step(test_batch)

            _loss = _loss.detach().cpu().numpy()

            dec_test_loss_per_epoch.append(_loss)

        dec_train_loss_per_epoch = np.mean(np.array(dec_train_loss_per_epoch))
        dec_test_loss_per_epoch = np.mean(np.array(dec_test_loss_per_epoch))
        dec_mel_loss_per_epoch = np.mean(np.array(dec_mel_loss_per_epoch))
        dec_perc_loss_per_epoch = np.mean(np.array(dec_perc_loss_per_epoch))
        ...
        loss_history_dec["dec test"].append(dec_test_loss_per_epoch)
        loss_history_dec["dec mel"].append(dec_mel_loss_per_epoch)
        loss_history_dec["dec perc"].append(dec_perc_loss_per_epoch)

        print(
            "decoder-only epoch {} : train {:01.4f} test {:01.4f} mel {:01.4f} perc {:01.4f} time {:01.2f}".format(
                epoch + 1,
                dec_train_loss_per_epoch,
                dec_test_loss_per_epoch,
                dec_mel_loss_per_epoch,
...
if save_weights == True:
    loss_history_dec = train_decoder_only(train_loader, test_loader, decoder_only_epochs)

    save_loss_as_csv(loss_history_dec, "results/histories/history_second_pass_{}.csv".format(decoder_only_epochs))
    save_loss_as_image(loss_history_dec, "results/histories/history_second_pass_{}.png".format(decoder_only_epochs))
...
audio_highlight_duration_sec = 20
audio_highlight_starts_sec = [20, 130, 240, 360, 480, 540, 660, 780]
...
@torch.no_grad()
def create_audio_space_representation(waveform_excerpts):

    encodings = []
    
    excerpt_count = waveform_excerpts.shape[0]
    
    for eI in range(0, excerpt_count, batch_size):
        
        waveform_excerpt = waveform_excerpts[eI:eI+batch_size]
 
        waveform_excerpt = torch.from_numpy(waveform_excerpt).to(device)
        
        waveform_excerpt = waveform_excerpt.unsqueeze(1)
        
        print("waveform_excerpt s ", waveform_excerpt.shape)
        
        mels_excerpt = vocos.feature_extractor(waveform_excerpt)
        
        print("mels_excerpt s ", mels_excerpt.shape)

        # use only the last mels
        mels_excerpt = mels_excerpt[:,:,:, -audio_mel_count_vae:]
  
        encoder_output_mu, encoder_output_std = encoder(mels_excerpt)
...
for audio_highlight_start_sec in audio_highlight_starts_sec:
    audio_highlight_ranges.append([ int(audio_highlight_start_sec * audio_sample_rate) // audio_waveform_increment, int((audio_highlight_start_sec + audio_highlight_duration_sec) * audio_sample_rate) // audio_waveform_increment ])
